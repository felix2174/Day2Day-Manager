                        ...
                        @foreach($projects as $project)
                            ...
                            @endphp
                                @php
                                    ...
                                    $projectSummary = $projectData['summary'] ?? [];
                                @endphp
                                <div class="gantt-project-row" data-project-id="{{ $project->id }}" style="border: 1px solid #e5e7eb; border-radius: 8px; background: white; padding: 16px; display: flex; flex-direction: column; gap: 16px;">
                                    <div style="display: grid; grid-template-columns: 260px 1fr; gap: 12px; align-items: center;">
                                        <div style="display: flex; align-items: center; gap: 8px;">
                                            ...
                                            <div style="position: relative; display: inline-block;">
                                                <button type="button" onclick="toggleProjectMenu({{ $project->id }})" style="background: none; border: none; cursor: pointer; padding: 4px 8px; color: #6b7280; font-size: 18px; line-height: 1; transition: all 0.2s;" onmouseover="this.style.color='#111827'" onmouseout="this.style.color='#6b7280'">
                                                    ⋮
                                                </button>
                                                <div id="projectMenu{{ $project->id }}" style="display: none; position: absolute; top: 100%; left: 0; background: white; border: 1px solid #d1d5db; border-radius: 8px; box-shadow: 0 8px 24px rgba(0,0,0,0.12); z-index: 1000; min-width: 220px; margin-top: 4px; overflow: hidden;">
                                    ...
                                    <div style="display: flex; flex-direction: column; gap: 8px;">
                                        @php
                                            // ========== MULTI-SOURCE EMPLOYEE LOADING ==========
                                            // Priority: 1) Assignments, 2) Team Assignments, 3) MOCO API
                                            
                                            $employeeDataByEmployee = []; // Use array instead of collection
                                            
                                            // SOURCE 1: Load assignments from database
                                            $freshAssignments = \App\Models\Assignment::where('project_id', $project->id)
                                                ->with(['employee' => function($query) {
                                                    $query->select('id', 'first_name', 'last_name', 'moco_id');
                                                }])
                                                ->whereNotNull('employee_id')
                                                ->orderBy('display_order')
                                                ->get();
                                            
                                            // Add assignments to employee data
                                            foreach ($freshAssignments as $assignment) {
                                                if (!$assignment->employee) continue;
                                                
                                                $empId = $assignment->employee_id;
                                                if (!isset($employeeDataByEmployee[$empId])) {
                                                    $employeeDataByEmployee[$empId] = [
                                                        'employee_id' => $empId,
                                                ...
                                                $employeeDataByEmployee[$empId]['tasks'][] = [
                                            ...
                                            // SOURCE 2: Load from MOCO API if no assignments exist
                                            if (empty($employeeDataByEmployee) && $project->moco_id) {
                                                try {
                                                    /** @var \App\Services\MocoService $mocoService */
                                                    $mocoService = app(\App\Services\MocoService::class);
                                                    $mocoTeam = $mocoService->getProjectTeam((int)$project->moco_id);
                                                    
                                                    if (is_array($mocoTeam)) {
                                                        foreach ($mocoTeam as $member) {
                                                            $mocoUserId = $member['user_id'] ?? null;
                                                            if (!$mocoUserId) continue;
                                                            
                                                            // Try to find local employee by moco_id
                                                            $localEmployee = \App\Models\Employee::where('moco_id', $mocoUserId)->first();
                                                            if (!$localEmployee) continue;
                                                            
                                                            $empId = $localEmployee->id;
                                                            if (!isset($employeeDataByEmployee[$empId])) {
                                                                $employeeDataByEmployee[$empId] = [
                                                                    'employee_id' => $empId,
                                                                    'employee_name' => $localEmployee->first_name . ' ' . $localEmployee->last_name,
                                                                    'tasks' => [],
                                                                    'source' => 'moco'
                                                                ];
                                                            }
                                                            ...
                                                            $projectEnd = $project->end_date ? \Carbon\Carbon::parse($project->end_date) : now()->addMonths(3);
                                                            
                                                            $employeeDataByEmployee[$empId]['tasks'][] = [
                                                                'task_name' => 'MOCO Zuweisung',
                                                    ...
                                                    \Log::warning('MOCO project team fetch failed for project ' . $project->id . ': ' . $e->getMessage());
                                                }
                                            }
                                        @endphp
                                        @forelse($employeeDataByEmployee as $employeeId => $employeeData)
                                            @php
                                                $employeeName = $employeeData['employee_name'];
                                                $employeeTasks = collect($employeeData['tasks']);
                                            ...
                                            <div style="display: grid; grid-template-columns: 260px 1fr; gap: 12px; align-items: center; margin-top: 8px;">
                                                <div style="height: 28px; display: flex; align-items: center; background: #f3f4f6; border: 1px solid #d1d5db; border-radius: 8px; padding: 0 12px; color: #111827; font-size: 13px; font-weight: 600; gap: 8px;">
                                                    ...
                                                    <div style="position: relative; display: inline-block;">
                                                        <button type="button" onclick="toggleEmployeeMenu({{ $project->id }}, {{ $employeeId }})" style="background: none; border: none; cursor: pointer; padding: 2px 4px; color: #6b7280; font-size: 16px; line-height: 1; transition: all 0.2s;" onmouseover="this.style.color='#111827'" onmouseout="this.style.color='#6b7280'">
                                                            ⋮
                                                        </button>
                                                        <div id="employeeMenu{{ $project->id }}_{{ $employeeId }}" style="display: none; position: absolute; top: 100%; left: 0; background: white; border: 1px solid #d1d5db; border-radius: 8px; box-shadow: 0 8px 24px rgba(0,0,0,0.12); z-index: 1000; min-width: 220px; margin-top: 4px; overflow: hidden;">
                                                            <div style="padding: 4px 0;">
                                                    ...
                                                    <span class="employee-name" 
                                                          ...
                                                          data-task-count="{{ count($employeeTasks) }}"
                                                          data-total-hours="{{ $employeeTasks->sum('weekly_hours') }}"
                                                          style="cursor: pointer;"
                                                          title="{{ $employeeName }} ({{ $source === 'assignment' ? 'Aufgaben' : 'MOCO' }})">
                                                        {{ $employeeName }}
                                                        @if($source !== 'assignment')
                                                            <span style="font-size: 10px; color: #6b7280; font-weight: 400; margin-left: 4px;">(MOCO)</span>
                                                        @endif
                                                    </span>
                                                </div>
                                                ...
                                                <div style="position: relative; height: 28px; border-radius: 8px; background: #f9fafb; border: 1px solid #e5e7eb;">
                                                    @foreach($employeeTasks as $taskIndex => $task)
                                                        @php
                                                            $taskStart = $task['start'];
                                                            $taskEnd = $task['end'];
                                                            $clampedStart = $taskStart->lt($timelineStart) ? $timelineStart->copy() : $taskStart;
                                                            $clampedEnd = $taskEnd->gt($timelineEnd) ? $timelineEnd->copy() : $taskEnd;
                                                            ...
                                                            $taskName = $task['task_name'] ?? 'Aufgabe';
                                                            $taskType = $task['type'];
                                                            
                                                            // Color coding by source: Blue for assignments, Gray for team/moco
                                                            $barColor = $taskType === 'assignment' ? '#0ea5e9' : '#9ca3af';
                                                        @endphp
                                                        <div class="project-task-bar" 
                                                             ...
                                                             data-start-date="{{ $taskStart->format('d.m.Y') }}"
                                                             data-end-date="{{ $taskEnd->format('d.m.Y') }}"
                                                             data-weekly-hours="{{ $hours ?? 0 }}"
                                                             data-duration-days="{{ $taskStart->diffInDays($taskEnd) + 1 }}"
                                                             data-source="{{ $taskType }}"
