                ...
                <div id="ganttContent" style="min-width: fit-content;">
                    <div id="timelineHeader" style="display: grid; grid-template-columns: 260px repeat({{ count($timelineMonths) }}, {{ $columnWidth }}px); gap: 1px; margin-bottom: 1px; position: relative;">
                        ...
                        @foreach($timelineMonths as $index => $monthData)
                            <div data-period-index="{{ $index }}" data-is-current-period="{{ $monthData['is_current'] ? 'true' : 'false' }}" style="background: {{ $monthData['is_current'] ? '#dbeafe' : '#f3f4f6' }}; padding: {{ $timelineUnit === 'day' ? '6px 4px' : ($timelineUnit === 'week' ? '8px 6px' : '12px') }}; text-align: center; font-weight: 600; color: {{ $monthData['is_current'] ? '#1e40af' : '#374151' }}; border: 1px solid {{ $monthData['is_current'] ? '#3b82f6' : '#e5e7eb' }}; font-size: {{ $timelineUnit === 'day' ? '10px' : ($timelineUnit === 'week' ? '11px' : '12px') }}; position: relative; min-width: {{ $columnWidth }}px;">
                                {{ $monthData['label'] }}
                            </div>
                        @endforeach
                    </div>
